...
本目录从 2026-06-05 的 6.5 mm 仿真基线演进而来，用于保存不会覆盖通用管线的实验版本。当前包含尺寸补偿 V2、SLM 安装偏移、输入光失配分析，以及中心空洞历史分析接口和已停用的 Zernike 补偿导出工具。
...
- 查阅历史球差/离焦导出工具：看 `README_ZERNIKE_COMPENSATION.md`；
...
2026-07-23 生成的 Z40/Z20 文件原为候选和扫描组。后续确认中心空洞由 PBS
在极高功率下损坏造成，这些补偿方案已经停用，也从未取代该 V2 基线。
...
- `export_zernike_compensated_slm.py`：历史 Zernike 补偿导出与回归复现；
- `export_zernike_compensated_y_sweep.py`：历史 V2+Zernike 相位 Y 扫描。
...
- [`README_ZERNIKE_COMPENSATION.md`](README_ZERNIKE_COMPENSATION.md)：已停用的球差/离焦补偿历史说明；
