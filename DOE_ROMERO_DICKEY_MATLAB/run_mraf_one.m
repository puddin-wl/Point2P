...
[~, input_amplitude, ~, aperture_mask] = gaussian_input_field(X_m, Y_m, cfg);
[phase_rd_unwrapped, phase_rd_wrapped, phase_info] = build_separable_phase_2d(X_m, Y_m, aperture_mask, cfg); %#ok<ASGLU>
...
fprintf(fid, 'y_free_reservoir: enable=%d, half_y=%.3f um, max_I=%.3f, pixels=%d\n', cfg.mraf.y_free_reservoir_enable, cfg.mraf.y_free_reservoir_half_y_um, cfg.mraf.y_free_reservoir_max_intensity, nnz(target.masks.y_free_reservoir));
fprintf(fid, 'wgs_y_edge_damping: enable=%d, factor=%.3f, I band=[%.3f, %.3f], pixels=%d\n', cfg.mraf.wgs_y_edge_damping_enable, cfg.mraf.wgs_y_edge_damping_factor, cfg.mraf.wgs_y_edge_damping_low, cfg.mraf.wgs_y_edge_damping_high, nnz(target.masks.wgs_y_edge_damping_band));
...
fields = {'core','signal','edge','target','free_noise','free','outer_suppress','noise','guard','finite_target'};
...
names = {'size50_x_um','size50_y_um','size_50_x_um','size_50_y_um','size13p5_x_um','size13p5_y_um', ...
    ...
    'transition_width_10_90_x_um','transition_width_10_90_y_um','core_rms','rms_90','peak_to_valley', ...
