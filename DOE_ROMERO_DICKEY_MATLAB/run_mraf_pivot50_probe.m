...
cases = {
    struct('variant_name', "pivot50_identity_baseline", 'target_edge_mode', "identity", 'inner', 1.00, 'outer', 1.00)
    struct('variant_name', "pivot50_p098_q102", 'target_edge_mode', "pivot50", 'inner', 0.98, 'outer', 1.02)
    struct('variant_name', "pivot50_p095_q105", 'target_edge_mode', "pivot50", 'inner', 0.95, 'outer', 1.05)
    struct('variant_name', "pivot50_p092_q108", 'target_edge_mode', "pivot50", 'inner', 0.92, 'outer', 1.08)
    struct('variant_name', "pivot50_p090_q110", 'target_edge_mode', "pivot50", 'inner', 0.90, 'outer', 1.10)
};
rows = [];
for k = 1:numel(cases)
    c = cases{k};
    variant = struct('variant_name', c.variant_name, 'preset_name', "caseC_stable", 'artifact_group', "pivot50_probe", ...
        'target_edge_mode', c.target_edge_mode, 'pivot50_inner_power', c.inner, 'pivot50_outer_power', c.outer, ...
        'n_iter', 10, 'rd_target_gamma', 1.0, 'free_threshold', 0.135, 'edge_low_threshold', 0.135, ...
        'mraf_factor', 1.0, 'phase_blend', 0.25, 'method', "GS-MRAF", 'use_wgs', false, 'wgs_exponent', 0);
    [cfg, result, fullprop] = run_mraf_one(variant); %#ok<ASGLU>
    m = result.mraf.metrics;
    rows = [rows; {k-1, string(c.variant_name), string(c.target_edge_mode), c.inner, c.outer, string(cfg.save_root), ...
...
t = cell2table(rows, 'VariableNames', {'case_index','variant_name','target_edge_mode','inner_power','outer_power','artifact_root', ...
...
t = add_pivot50_status(t);

stamp = string(datetime('now', 'Format', 'yyyyMMdd-HHmmss'));
summary_root = fullfile(project_root, 'artifacts', 'pivot50_probe', char(stamp + "_pivot50_probe_summary"));
if ~exist(summary_root, 'dir'), mkdir(summary_root); end
writetable(t, fullfile(summary_root, 'pivot50_probe_metrics.csv'));
write_pivot50_summary(fullfile(summary_root, 'pivot50_probe_summary.txt'), t);
plot_pivot50_summary(fullfile(summary_root, 'pivot50_probe_summary_plot.png'), t);
plot_pivot50_profile_overlay(fullfile(summary_root, 'pivot50_profile_overlay.png'), t);
plot_pivot50_remap_curves(fullfile(summary_root, 'pivot50_remap_curves.png'), t);
fprintf('\nPivot50 probe summary written to:\n%s\n', summary_root);
end

function t = add_pivot50_status(t)
b = t(1,:);
...
for i=1:height(t)
    ...
    if t.side_lobe_peak_x_rel_to_core(i) > b.side_lobe_peak_x_rel_to_core + 0.03 || t.side_lobe_peak_y_rel_to_core(i) > b.side_lobe_peak_y_rel_to_core + 0.03, warn(end+1)="side_lobe"; end %#ok<AGROW>
    if t.efficiency_inside_signal(i) < b.efficiency_inside_signal * 0.98, warn(end+1)="efficiency"; end %#ok<AGROW>
    if t.size13p5_x_um(i) > b.size13p5_x_um + 3 || t.size13p5_y_um(i) > b.size13p5_y_um + 3, warn(end+1)="size13p5_expansion"; end %#ok<AGROW>
    if ~isempty(hard)
...
function write_pivot50_summary(path,t)
fid=fopen(path,'w'); cleanup=onCleanup(@() fclose(fid));
fprintf(fid,'Pivot50 probe summary\n=====================\n\n');
fprintf(fid,'Fixed: free_threshold=0.135, gamma=1.0, n_iter=10, mraf_factor=1.0, phase_blend=0.25, GS-MRAF, WGS off.\n');
fprintf(fid,'Pivot50 remap anchors: I=0->0, I=0.5->0.5, I=1->1.\n\n');
for i=1:height(t)
    fprintf(fid,'%s mode=%s inner=%.3f outer=%.3f  %s\n',t.variant_name(i),t.target_edge_mode(i),t.inner_power(i),t.outer_power(i),t.pass_fail_reason(i));
...
ok=t(t.status~="FAIL",:);
if isempty(ok)
    fprintf(fid,'No pivot50 candidate passed hard constraints. Keep identity baseline.\n');
else
    score=ok.delta_transition_x_um+ok.delta_transition_y_um+50*max(ok.delta_shoulder_x,0)+50*max(ok.delta_shoulder_y,0)+0.2*abs(ok.delta_size50_x_um)+0.2*abs(ok.delta_size50_y_um);
    [~,idx]=min(score); fprintf(fid,'Suggested conservative candidate: %s\n',ok.variant_name(idx));
end
end

function plot_pivot50_summary(path,t)
...
sgtitle('Pivot50 probe: identity baseline vs anchored edge remaps'); exportgraphics(fig,path,'Resolution',150); close(fig);
end
...
function plot_pivot50_profile_overlay(path,t)
...
format_axis(ax1,'X center profiles: RD, identity, pivot50 cases','x (um)',[-380 380]); format_axis(ax2,'Y center profiles: RD, identity, pivot50 cases','y (um)',[-230 230]);
sgtitle('Pivot50 profile overlay: check 50% crossing, 13.5% crossing, shoulder'); exportgraphics(fig,path,'Resolution',150); close(fig);
end
...
function plot_pivot50_remap_curves(path,t)
fig=figure('Visible','off','Color','w'); I=linspace(0,1,501); plot(I,I,'k--','LineWidth',1.0,'DisplayName','identity'); hold on; colors=lines(height(t));
for i=1:height(t)
    if t.target_edge_mode(i)=="pivot50"
        Y=pivot_curve(I,t.inner_power(i),t.outer_power(i)); plot(I,Y,'-','Color',colors(i,:),'LineWidth',1.2,'DisplayName',char(t.variant_name(i)));
    end
end
plot(0.5,0.5,'ro','MarkerFaceColor','r','DisplayName','0.5 anchor'); grid on; axis([0 1 0 1]); xlabel('I_{rd,norm}'); ylabel('I_{env}'); title('Pivot50 remap curves'); legend('Location','best'); exportgraphics(fig,path,'Resolution',150); close(fig);
end
function Y=pivot_curve(I,p,q)
Y=zeros(size(I)); outer=I<0.5; Y(outer)=0.5*(2*I(outer)).^q; Y(~outer)=0.5+0.5*(2*I(~outer)-1).^p;
end