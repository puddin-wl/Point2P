...
---
所有 matplotlib 出图函数:
  - target 模板 (intensity, amplitude, constraint, profiles)
  - mask 区域可视化
  - 相位图 (初始/精修)
  - 焦面强度图 + 剖面对比 (初始 vs MRAF vs WGS)
  - WGS 权重可视化 (2D 颜色图 + histogram + X-only profile)
  - 收敛曲线 (RMS, efficiency, background, size50)
"""
...
def _extent(x_um: np.ndarray, y_um: np.ndarray) -> list[float]:
    """返回 imshow 的 extent 参数 [x0, x1, y0, y1]."""
...
def plot_target(target: Any, outdir: str | Path, dpi: int = 150) -> None:
    """绘制目标模板: I_full, I_constraint, A_full 三张图."""
    outdir = Path(outdir)
...
def plot_masks(target: Any, outdir: str | Path, dpi: int = 150) -> None:
    """绘制 4 色离散 mask 图 (flat/edge_lock/free/far_bg)."""
    outdir = Path(outdir)
...
def plot_target_profiles(target: Any, outdir: str | Path, dpi: int = 150) -> None:
    """绘制 target 中心剖面 + release_level + a0/a50/a1/a2 几何标记."""
    outdir = Path(outdir)
    release_level = float(target.params.get("release_level", np.exp(-2.0)))
...
def plot_intensity(
    ...
    """保存归一化重建强度 2D 图 + flat/size50/outer edge 矩形标记."""
...
def plot_phase(phase: np.ndarray, outpath: str | Path, title: str, dpi: int = 150) -> None:
    """保存包裹相位 2D 图 (twilight colormap, 0~2π rad)."""
...
def plot_profiles_compare(
    ...
    """初始 vs 精修中心剖面对比 (I_full template + I_constraint + profiles)."""
    outdir = Path(outdir)
...
def plot_profiles_compare_mraf_xy_xonly(
    ...
    """MRAF → WGS-XY → WGS-Xonly 三阶段剖面对比 (xy_then_x 策略专用)."""
    outdir = Path(outdir)
    mraf_n = normalize_intensity(mraf_intensity, masks["mask_flat"], mode="flat_mean")
...
def plot_wgs_x_weights(
    ...
    """绘制 x-only WGS 权重: 1D profile + 在 mask_flat 中的广播 2D 图."""
    outdir = Path(outdir)
...
def plot_convergence(metrics_history: list[dict[str, Any]], outdir: str | Path, dpi: int = 150) -> None:
    """绘制收敛曲线: RMS / 效率 / 背景占比 / size50 随迭代变化."""
    outdir = Path(outdir)
