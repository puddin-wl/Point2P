...
f=200mm, SLM 1024x1024 @ 17um pitch.
Computational grid: 2048x2048 (refinement), then resample to SLM.

整体流程:
1. 加载 phase0 (从 .mat) 或生成 smoke-test 随机相位
2. 构建 RTAD 升余弦矩形目标 + mask 区域
3. 构建高斯输入振幅 (DOE 平面)
4. 调用 run_refinement() 跑 GS/MRAF/WGS 迭代
5. 保存相位、重建图像、指标曲线、图表、报告
6. (可选) 运行 Python 诊断
"""
...
def parse_args() -> argparse.Namespace:
    """解析 50+ 个 CLI 参数, 覆盖 config 中的任何默认值."""
    ...
    parser.add_argument("--beam-diameter", type=float, default=None, help="Input Gaussian 1/e^2 beam diameter in mm. Overrides config default.")
...
def apply_overrides(config: dict, args: argparse.Namespace) -> dict:
    ...
    命令行参数可以覆盖 CONFIG 中的任何默认值。
    只有用户显式传入的参数才会覆盖 (≠None)。
    """
...
def load_or_make_phase(cfg: dict) -> tuple[np.ndarray, dict, np.ndarray | None, np.ndarray | None]:
    ...
    优先从 .mat 文件加载, 支持 h5py (v7.3) 和 scipy.io (v7) 两种格式。
    如果没有指定 --phase-mat, 则生成随机相位作为 smoke test (仅用于调试)。
    """
...
def make_output_dir(cfg: dict, args: argparse.Namespace) -> Path:
    """创建时间戳输出目录 (artifacts/<timestamp>_rtad_mraf_gs...)."""
    if args.outdir:
...
def write_report(outdir: Path, cfg: dict, phase_info: dict, target, result) -> None:
    """输出纯文本精修报告 (report.txt), 包含参数/指标/权重统计."""
    lines = [
        "RTAD MRAF/GS Python refinement report",
        f"SLM: 1024x1024 @ 17um pitch (f=200mm)",
        "",
        ...
        f"  mraf_factor=0 fully attenuates the free region; mraf_factor=1 keeps it unchanged.",
        ...
        "  WGS updates only mask_flat weights.",
        "",
        "Run warnings/info:",
...
def main() -> int:
    """主入口: 加载 phase0 → 构建目标 → WGS 迭代 → 保存输出 + 绘图 + 报告 (整管线)."""
    args = parse_args()
    cfg = apply_overrides(CONFIG, args)
    outdir = make_output_dir(cfg, args)

    print(f"Output directory: {outdir}")
    # 加载 phase0 或生成 smoke-test 相位
