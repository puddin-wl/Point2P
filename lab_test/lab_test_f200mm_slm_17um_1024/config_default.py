"""Default configuration for f=200mm, SLM 1024x1024 @ 17um pitch.
---
本配置文件定义了 f=200mm 仿真验证管线的全部默认参数。
分为 6 个区块:
  physical  - 物理参数 (波长、焦距、光束直径、通光孔径)
  grid      - 计算网格 (N, 焦面采样间距)
  target    - 目标光斑 (尺寸、过渡区宽度、截断模式)
  refinement - 精修算法 (方法、迭代、WGS 权重参数、背景衰减)
  paths     - 输入路径 (phase0 文件来源)
  runtime   - 运行时 (GPU/CuPy, 随机种子)
  slm       - SLM 参数 (分辨率、像素尺寸)

命令行传入的参数会覆盖这里的默认值 (见 run_rtad_mraf_gs_case.py:apply_overrides)。
"""
