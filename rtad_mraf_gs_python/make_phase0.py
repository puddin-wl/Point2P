"""Generate Romero-Dickey initial phase for f=429mm (2048x2048 computational grid).

Romero-Dickey 解析法: 在高斯光束与矩形平顶之间求解稳相能量守恒方程,
得到可分离的 DOE 初始相位 φ(x,y) = φ_x(x) + φ_y(y)。
每个方向的相位为 φ(u) = β × [ξ√π/2·erf(ξ) + ½exp(-ξ²) - ½], 其中 ξ = x / ri。
β 越大 → 越接近几何光学 → 稳相近似越精确 → 初始相位越接近真解。
...
def gaussian_input_field(X_m, Y_m, input_1e2_radius_m, aperture_radius_m):
    """构建 DOE 平面高斯振幅场 A(r)=exp(-r²/w²), 加通光孔径掩膜."""
    r2 = X_m**2 + Y_m**2
...
def romero_dickey_phase_1d(x_m, ri_m, Ro_m, lambda_m, f_m):
    """Romero-Dickey 一维解析相位: φ(u) = β × [ξ√π/2·erf(ξ) + ½exp(-ξ²) - ½]."""
    xi = x_m / ri_m
...
def main():
    """入口: 解析参数 → 构建网格/光束 → 生成 RD 相位 → 验证 FFT → 保存 .mat."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--beam", type=float, required=True, help="Beam 1/e^2 diameter in mm")
    parser.add_argument("--out", default=None, help="Output directory")
    parser.add_argument("--f", type=float, default=429e-3, help="Focal length in m")
    ...
    # 快速检查: 中心剖面 size50 是否接近目标值
    cy = N // 2
    prof_x = I_norm[cy, :]
    prof_y = I_norm[:, N // 2]
    def crossing_width(axis_um, profile, level):
        """计算剖面在 threshold level 处的 crossing 全宽."""
        mid = len(profile) // 2
