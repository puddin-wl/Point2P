...
\begin{frame}{物理基础 \ \ Romero-Dickey 点对点法}
  {\footnotesize Romero \& Dickey, ``Lossless laser beam shaping,'' \textit{J. Opt. Soc. Am. A}, 13(4), 751--760 (1996).}
...
\begin{frame}{物理基础 \ \ Romero-Dickey 相位公式与初始相位}
  \begin{columns}
    ...
    \[
      \boxed{\;
      \phi(\xi) = \beta \cdot \left[
        \xi\frac{\sqrt{\pi}}{2}\,\mathrm{erf}(\xi)
        + \frac{1}{2}e^{-\xi^{2}}
        - \frac{1}{2}
      \right]
      \;}
    ...
    \column{0.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{phase0.png}\\
...
\begin{frame}{物理基础 \ \ RTAD 目标与截断约束}
  \begin{columns}
    ...
    \column{0.45\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{target_constraint_intensity.png}\\
      {\footnotesize RTAD 约束目标}
      \vspace{0.3cm}
      \includegraphics[width=\textwidth]{masks.png}\\
      {\footnotesize 区域划分 (masks)}
    \end{center}
  \end{columns}
\end{frame}
...
\begin{frame}{物理基础 \ \ WGS 优化算法}
  ...
  \[
    \boxed{\;
    w \leftarrow w\cdot\left(\frac{\langle|E_{\mathrm{flat}}|\rangle}{|E|}\right)^{\gamma},\quad
    w = \mathrm{clip}(w,\,0.5,\,2.0),\quad
    w\; /\!= \langle w\rangle
    \;}
  \]
\end{frame}
...
\begin{frame}{物理基础 \ \ WGS 优化结果}
  \begin{columns}
    ...
    \column{0.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{refined_reconstruction_intensity.png}\\
  ...
  \vspace{0.3cm}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{center_profiles_compare.png}\\
...
\begin{frame}{物理基础 \ \ 优化策略要点}
  \begin{columns}
    ...
    \begin{itemize}
      \item method = wgs
      \item strategy = flat\_local
      \item iterations = 200
      \item feedback\_exponent = 0.8
      \item weight range = [0.5, 2.0]
      \item mraf\_factor = 0.8
      \item bg\_factor = 1.0
    \end{itemize}

    \column{0.45\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{convergence_metrics.png}\\
      {\footnotesize 收敛曲线}
    \end{center}
  \end{columns}
...
\begin{frame}{真实光路容差评估}
  ...
  \begin{center}
    每种误差独立扫描 (mild + stress)，双参数组合扫描用于定位耦合效应
  \end{center}
\end{frame}
...
\begin{frame}{问题  --  长边中间内凹  $\to$  椭圆度 $D_x > D_y$}
  \begin{columns}
    ...
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_nominal_Dx5p0_Dy5p0.png}\\
      {\footnotesize nominal: $D_x\!=\!D_y\!=\!5.0$mm}
    \end{center}
    \column{0.33\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_Dx5p5_Dy4p5.png}\\
      {\footnotesize $D_x\!=\!5.5$,\, $D_y\!=\!4.5$mm \quad 内凹+5.8$\mu$m}
    \end{center}
    \column{0.33\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_Dx6p0_Dy4p0.png}\\
...
\begin{frame}{问题  --  四条边都内凹  $\to$  负离焦 + 光斑偏大}
  \begin{columns}
    ...
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_nominal_p3.png}\\
      {\footnotesize nominal}
    \end{center}
    \column{0.25\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_df=m0p50mm_sz=5p5mm.png}\\
      {\footnotesize $df\!=\!-0.5$, $sz\!=\!5.5$mm}
    \end{center}
    \column{0.25\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_df=m1p00mm_sz=6p0mm.png}\\
      {\footnotesize $df\!=\!-1.0$, $sz\!=\!6.0$mm}
    \end{center}
    \column{0.25\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_df=p0p50mm_sz=4p0mm.png}\\
...
\begin{frame}{问题  --  长边能量缺失 + 短边倾斜  $\to$  beam\_offset}
  \begin{columns}
    \column{0.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_nominal_p2.png}\\
      {\footnotesize nominal}
    \end{center}
    \column{0.5\textwidth}
    \begin{center}
      \includegraphics[width=\textwidth]{intensity_ox=0p3_oy=0p3.png}\\
...
\begin{frame}{误差对照总览}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{OVERLAY/overlay_problem1.png}\\
    {\footnotesize 问题 1 中心剖面叠加 —— 椭圆度逐级增大，内凹加深}
  \end{center}
\end{frame}

% ===================================================================
\begin{frame}{误差对照总览 (续)}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{OVERLAY/overlay_problem3.png}\\
    {\footnotesize 问题 3 中心剖面叠加 —— 实线 = 正离焦，虚线 = 负离焦}
  \end{center}
\end{frame}
