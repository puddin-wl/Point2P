...
### Stage 4: Captured Image Analysis

Use `fig_analysis/analyze_captured.py` to measure flat-top spot images from the lab.
Supports .mat (MATLAB), .bmp, .png, .tif.

```bash
python fig_analysis/analyze_captured.py <image_path> [--pixel-um 3.45]
```

**Algorithm**: gradient-edge detection. The script:
1. Finds the bright region center via intensity-weighted center of mass
2. Extracts x/y center profiles and computes gradient magnitude
3. Locates the two dominant gradient peaks on each side of center — these bracket the flat-top region
4. Computes the flat-level as the median intensity between gradient edges (robust against central hotspots)
5. Finds size50/size90/size13.5 crossings via linear interpolation relative to this flat level

Output in `fig_analysis/output/`: analysis PNG with annotated profiles, JSON summary with all metrics.

**Why not peak-based thresholding**: a central hotspot biases the peak upward,
making the 50% threshold too high and under-estimating the flat-top width.
The gradient-edge method uses edge geometry rather than peak intensity to
locate the flat region, then measures the flat level within those bounds.

## Important Lessons

### SLM phase resampling: use complex-field interpolation

When converting computational phase (2048×2048, dx_doe ≈ 20-45 μm) to SLM
resolution (1920×1080, 6.4 μm), never interpolate the wrapped phase [0, 2π)
directly. The 2π→0 discontinuity causes cubic-spline ringing artifacts.

```python
# WRONG — ringing at phase wraps
phase_slm = zoom(phase_cropped, (zy, zx), order=3)

# CORRECT — complex field is continuous across phase wraps
c = np.exp(1j * phase_cropped)
phase_slm = np.arctan2(zoom(c.imag, (zy, zx), order=3),
                       zoom(c.real, (zy, zx), order=3))
```

### Python phase0 needs --no-swap-phase-xy

When phase0 is generated by `make_phase0.py` (numpy convention), the default
`swap_phase_xy: True` incorrectly transposes x/y. Always pass `--no-swap-phase-xy`.
For MATLAB-generated phase0, keep the default `swap_phase_xy: True`.

### Exposure: keep flat-top peak well below 255

Camera images with saturated pixels (value=255) cannot be analyzed accurately.
Reduce laser power or exposure time so the flat-top peak is ~200 (8-bit) or
lower, leaving headroom for the true peak to be measured.
